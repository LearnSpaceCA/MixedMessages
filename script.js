let mixedMessages = {
    birth: [
        ...
        '“The nature of finite things as such is to have the seed of passing away as their essential being: the hour of their birth is the hour of their death.” ― Georg Wilhelm Friedrich Hegel',
    ...
    death: [
        ...
        '"It is solely by risking life that freedom is obtained" ― Georg Wilhelm Friedrich Hegel',
    ...
    show() {
        let birthMessage = this.birth[Math.floor(Math.random() * Object.keys(this).length)];
        let lifeMessage = this.life[Math.floor(Math.random() * Object.keys(this).length)];
        let deathMessage = this.death[Math.floor(Math.random() * Object.keys(this).length)];
        if (!deathMessage || !lifeMessage || !birthMessage) {
            return 'Quote missing in a category. Please fill it.';
        } else {
            let messageRabndom = "BIRTH:" + birthMessage + "\n" + "LIFE:" + lifeMessage + "\n" + "DEATH:" + deathMessage;
            return messageRabndom;
        }
        
    }
}
console.log(mixedMessages.show());